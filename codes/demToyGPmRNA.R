...
library(MASS)
library(plot3D)
library(NORMT3)   # for wofz function
...
kernOpts <- GPmRNAOpts() # default options
...
nb_exp <- 1
toyName <- paste("demToy", nb_exp, "GPmRNA", sep = "")
...
if (file.exists(dataName)){
  ...
  jointCov@par <- c(sigma2=1, theta_t=0.3, theta_x=0.3, decay=0.1, diffusion=0.01, sensitivity = 1)
  K <- covMat(object = jointCov, X = X, Xnew = X)
  set.seed(2); YUsamples <- mvrnorm(nsamples, rep(0,nrow(K)), K)
